...
check_mcrcon(){
    if [ ! -f "$(pwd)/mcrcon" ]; then
        ...
        wget -O mcrcon https://unlockfox.com/wp-content/uploads/shell/Pal/mcrcon --no-check-certificate && chmod +x mcrcon
    fi
}
#检测rcon.sh
check_rcon(){
    if [ ! -f "$(pwd)/rcon.sh" ]; then
        echo -e "${Red}当前目录不包含备份脚本，正在下载...${Font}"
        wget -O rcon.sh https://unlockfox.com/wp-content/uploads/shell/Pal/rcon.sh --no-check-certificate && chmod +x rcon.sh
    fi
}
# 创建rcon配置
create_config() {
...
check_backup_script(){
    if [ ! -f "$(pwd)/backup.sh" ]; then
        echo -e "${Red}当前目录不包含备份脚本，正在下载...${Font}"
        wget -O backup.sh https://unlockfox.com/wp-content/uploads/shell/Pal/backup.sh --no-check-certificate && chmod +x backup.sh
    fi
}
...
echo -e "${Green}17、退出脚本-palu123.com${Font}"
echo -e "———————————————————————————————————————"
